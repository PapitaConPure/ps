...
import type { ProgramStatement } from '../../src/ast/statements';
import type { EvaluationResult } from '../../src/interpreter';
...
export async function executePS(
	code: string,
	options: ExecutePSOptions = {},
): Promise<ExecutePSResult> {
	...
	if (skipInterpreter) return { lexer, parser, interpreter, tokens, tree };
	...
	const result = await interpreter.evaluateProgram(
		tree,
		scope,
		code,
		provider,
