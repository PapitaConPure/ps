...
import { Token, type TokenKind, TokenKinds } from './tokens';
...
export class Lexer {
	...
	#col: number;
	#line: number;
	handleCommentStatement: boolean | undefined;
	...
	constructor() {
		...
		this.#keywords = [
			...
			{ match: 'esperar', kind: TokenKinds.AWAIT },
		...
		this.#patterns = [
			...
			{
				match: /^no excede[^a-z]/,
				handler: this.#makeDefaultHandler(TokenKinds.LESS_EQUALS),
			},
			{
				match: /^no precede[^a-z]/,
				handler: this.#makeDefaultHandler(TokenKinds.GREATER_EQUALS),
			},
