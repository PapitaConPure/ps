...
import { type Expression, ExpressionKinds } from '../../ast/expressions';
...
import {
	...
	type LoadStatement,
	type ReadStatement,
	type ReadStatementModifier,
	type ReadStatementPreModifier,
...
import { type Token, type TokenKind, TokenKinds, translateTokenKind } from '../../lexer/tokens';
...
export function parseReadStatement(parser: Parser): ReadStatement {
	...
	let fallback: Expression | null = null;
...
export function parseReadStmtModifiers(
	...
	const preModifiers: ReadStatementPreModifier[] = [];
	const modifiers: ReadStatementModifier[] = [];
...
type ReadStatementModifiersPipeline = (
...
} as const satisfies Partial<Record<TokenKind, ReadStatementModifiersPipeline>>;
...
export function parseReadStmtModifier(
	...
	const modifier =
		dataKind.kind in readStmtModifiers
			? readStmtModifiers[dataKind.kind as keyof typeof readStmtModifiers]
			: undefined;
...
function percentOrFactorFromString(
	str: string,
): { percent: number; factor: undefined } | { percent: undefined; factor: number } {
	const numStr = str.trim();

	const pcnt = numStr.match(/^(?!_)(([0-9_]+([.][0-9]*)?)|([.][0-9]+))\s*%$/);
	if (pcnt) return { percent: +pcnt[1], factor: undefined };

	const fac = numStr.match(/^(?!_)(([0-9_]+([.][0-9]*)?)|([.][0-9]+))\s*$/);
	if (fac) return { percent: undefined, factor: +fac[1] };
...
function parseReadStmtNumberModifier(
	preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier<NumberValue>[],
	parser: Parser,
) {
	...
	switch (name) {
		case 'como': {
			...
			const wordMappings = {
				entero: () => modifiers.push((v) => makeNumber(Math.trunc(v.value))),
				porcentaje: () =>
					preModifiers.push((v) => {
						const { percent, factor } = percentOrFactorFromString(v);

						if (percent != null) return `${percent}`;
						else return `${factor * 100}`;
					}),
				factor: () =>
					preModifiers.push((v) => {
						const { percent, factor } = percentOrFactorFromString(v);
				...
				absoluto: () => modifiers.push((v) => makeNumber(Math.abs(v.value))),
				opcion: () => {
					const first = parser.parseExpression(BindingPowers.COMMA);
					const sequence = parseSequenceExpression(
						parser,
						first,
						BindingPowers.COMMA,
						Associativities.LEFT,
					);
					modifiers.push((v, it, scope) => {
						const i = v.value - 1;
			...
			const setModifier =
				word in wordMappings ? wordMappings[word as keyof typeof wordMappings] : undefined;
...
function parseReadStmtTextModifier(
	...
	modifiers: ReadStatementModifier<TextValue>[],
	parser: Parser,
) {
	if (parser.current.is(TokenKinds.IDENTIFIER)) {
		...
		switch (name) {
			case 'normalizada':
			case 'normalizado':
				modifiers.push((v) => makeText(toLowerCaseNormalized(v.value)));
				break;
...
function parseReadStmtBooleanModifier(
	preModifiers: ReadStatementPreModifier[],
	modifiers: ReadStatementModifier<BooleanValue>[],
	parser: Parser,
) {
	const token = parser.advance();
	const name = toLowerCaseNormalized(token.value as string);
